...
# *******************         Experimental Design
            ...
            if not binary:
             ...
             # Jing sparse solution            
            self.A_inv = np.linalg.pinv(pulls.T@pulls+np.identity(pulls.shape[1]))              
            theta = self.A_inv@pulls.T@rewards
            self.omega = (-abs(theta.flatten())).argsort()[:self.sparsity] 
            
            self.diff = np.setdiff1d(np.arange(self.d), self.omega)
            
            theta[self.diff] = 0
            if np.linalg.norm(theta) != 0:
                self.alpha = np.linalg.norm(self.theta_star)/np.linalg.norm(theta)
            else:
                print("zero!")
            self.theta_hat = self.alpha * theta
...
#             print(self.active_arms)
    ...
    def optimal_allocation(self): # experimental design
...
#             print("drop_arms True 2")
            ...
            for arm_idx in active_arms:
                ...
                for arm_idx_prime in active_arms:
                    ...
                    y = arm_prime - arm
                    if y.T@self.theta_hat >= np.sqrt(2*y.T@self.pull@self.A_inv@y*np.log(2*self.K**2/self.delta_t)):
